    ...
    public class TermsAndConditionService: ITermsAndConditionService
    {
        ...
        public IEnumerable<TermsAndCondition> InsertTerms(TermsAndCondition termsAndCondition)
        {
            ...
            try
            {
                valid = true;
                int mxc = 0;
                mxc = (int)termsrepository.GetAsQueryable().Where(k => k.TermsAndConditionTermsId != null).Select(x => x.TermsAndConditionTermsId).Max();
                if (mxc == null) { mxc = 1; } else { mxc = mxc + 1; }

                termsAndCondition.TermsAndConditionTermsId = mxc;
